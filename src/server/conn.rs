    ...
    E: H2Exec<S::Future, B>,
{
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<Self::Output> {
        // Safety: O_O
        //
        // This *could* move in `upgrade_h2`, but that should only happen
        // when there isn't currently a service future inside the dispatcher,
        // so things should be just fine? lol.
        let me = unsafe { self.get_unchecked_mut() };

        loop {
            match ready!(unsafe { Pin::new_unchecked(me.conn.as_mut().unwrap()) }.poll(cx)) {
                Ok(done) => {
                ...
                Err(e) => {
                    match *e.kind() {
                        Kind::Parse(Parse::VersionH2) if me.fallback.to_h2() => {
                            me.upgrade_h2();
                            continue;
                        }
                        _ => return Poll::Ready(Err(e)),
...
pub(crate) mod spawn_all {
        ...
        W: Watcher<I, S, E>,
    {
        ...
        fn poll(mut self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<Self::Output> {
            ...
            // an `async fn`, and much safe. Woe is me.
...
mod upgrades {
        ...
        E: super::H2Exec<S::Future, B>,
    {
        ...
        fn poll(mut self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<Self::Output> {
            let me = unsafe { self.get_unchecked_mut() };
            loop {
                match ready!(unsafe { Pin::new_unchecked(me.inner.conn.as_mut().unwrap()) }.poll(cx)) {
                    Ok(proto::Dispatched::Shutdown) => return Poll::Ready(Ok(())),
                    Ok(proto::Dispatched::Upgrade(pending)) => {
                        unimplemented!("Upgraded pending!");
                        // TODO: this stuff is probably a No-No for unsafe pinning...
                        /*
                    ...
                    Err(e) => {
                        match *e.kind() {
                            Kind::Parse(Parse::VersionH2) if me.inner.fallback.to_h2() => {
                                me.inner.upgrade_h2();
                                continue;
                            }
                            _ => return Poll::Ready(Err(e)),
